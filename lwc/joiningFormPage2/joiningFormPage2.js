...
import ssc from '@salesforce/apex/NewCandidateDetails.ssc';
   ...
   sscRecord={
    ...
    handleInputChange(event)
    {
        if(event.target.name == 'stateofSSCschool')
            {
            this.sscRecord.stateofSSCschool = event.target.value;
            console.log('stateofSSCschool--->' , this.sscRecord.stateofSSCschool);
            }
        if(event.target.name=='stateofSSCboard')
            this.sscRecord.stateofSSCboard = event.target.value;
        if(event.target.name=='nameofSSCboard')
            this.sscRecord.nameofSSCboard = event.target.value;
        ...
        if(event.target.name=='datemonthyearofpassing')
            this.sscRecord.datemonthyearofpassing = event.target.value;
    }
    validateForm() {
        let isValid = true;
    ...
    handleClick() {
            ...
            console.log('stateofSSCschool is---' + this.sscRecord.stateofSSCschool);
            ...
            console.log('date month year of passing----' + this.sscRecord.datemonthyearofpassing);
            console.log('Entered record-----' + JSON.stringify(this.sscRecord));
            if (this.sscRecord) {
                personalInfo({ wrapRecord: JSON.stringify(this.sscRecord) })
                        .then(result => {
